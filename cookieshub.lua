...
do
    ...
    local function enableFPSBoost()
        if FPSBoost.running then return end
        FPSBoost.running = true
        local camera = workspace.CurrentCamera
        if camera then
            FPSBoost.originalFOV = camera.FieldOfView
            camera.FieldOfView = 107
        end
        FPSBoost.connection = RunService.RenderStepped:Connect(function()
            if not FPSBoost.running then return end
            local cam = workspace.CurrentCamera
            if cam then cam.FieldOfView = 107 end
        end)
    end

    local function disableFPSBoost()
        FPSBoost.running = false
        if FPSBoost.connection then
            pcall(function() FPSBoost.connection:Disconnect() end)
            FPSBoost.connection = nil
        end
        local camera = workspace.CurrentCamera
        if camera then camera.FieldOfView = FPSBoost.originalFOV or 70 end
        FPSBoost.originalFOV = nil
    end
    ...
    local function enableAntiLag()
        if AntiLag.running then return end
        AntiLag.running = true
        AntiLag.originalBrightness = Lighting.Brightness
        AntiLag.originalClockTime = Lighting.ClockTime
        AntiLag.originalOutdoorAmbient = Lighting.OutdoorAmbient
        ...
        AntiLag.connection = workspace.DescendantAdded:Connect(function(obj)
            if AntiLag.running then applyAntiLagDerender(obj) end
        end)
    end

    local function disableAntiLag()
        AntiLag.running = false
        if AntiLag.connection then
            pcall(function() AntiLag.connection:Disconnect() end)
            AntiLag.connection = nil
        end
        pcall(function()
            if AntiLag.originalBrightness ~= nil then Lighting.Brightness = AntiLag.originalBrightness end
            if AntiLag.originalClockTime ~= nil then Lighting.ClockTime = AntiLag.originalClockTime end
            if AntiLag.originalOutdoorAmbient ~= nil then Lighting.OutdoorAmbient = AntiLag.originalOutdoorAmbient end
        end)
    end

    _G.__CookiesPerformance = {
        enableFPSBoost = enableFPSBoost,
        disableFPSBoost = disableFPSBoost,
        enableAntiLag = enableAntiLag,
        disableAntiLag = disableAntiLag,
    }
end

local function enableFPSBoost()
    local api = _G.__CookiesPerformance
    if api and api.enableFPSBoost then api.enableFPSBoost() end
end

local function disableFPSBoost()
    local api = _G.__CookiesPerformance
    if api and api.disableFPSBoost then api.disableFPSBoost() end
end

local function enableAntiLag()
    local api = _G.__CookiesPerformance
    if api and api.enableAntiLag then api.enableAntiLag() end
end

local function disableAntiLag()
    local api = _G.__CookiesPerformance
    if api and api.disableAntiLag then api.disableAntiLag() end
end
...
local SongPage = createPage("Song")
...
local SongParent = game:GetService("SoundService")
local SongAsset = getcustomasset or getsynasset

local Songs = {
 {title="Tuff Song",url="https://files.catbox.moe/rvf2vy.mp3",file="tuffsong.mp3",soundName="COOKIES_TuffSong",volume=0.75},
 {title="orula",url="https://files.catbox.moe/v20ko9.mp3",file="friosong.mp3",soundName="COOKIES_Orula",volume=0.85},
 {title="to the O",url="https://file.garden/algLafWA1jk8WMfK/King%20Von%20-%20Took%20Her%20To%20The%20O%20(Lyrics)(MP3_160K).mp3",file="cookies_to_the_o.mp3",soundName="COOKIES_ToTheO",volume=0.75},
 {title="LAJA",url="https://file.garden/algLafWA1jk8WMfK/LAJA%20-%20NADIE%20TA%20FRIO%20(Letra)(MP3_160K).mp3",file="cookies_laja.mp3",soundName="COOKIES_LAJA",volume=0.75},
 {title="Lucid Dreams",url="https://file.garden/algLafWA1jk8WMfK/Lucid%20Dreams%20-%20Clean%20-%20Juice%20WRLD(MP3_160K).mp3",file="cookies_lucid_dreams.mp3",soundName="COOKIES_LucidDreams",volume=0.75},
 {title="NUTS",url="https://archive.org/download/li-l-peep-nuts-feat.-lil-skil-extended_202011/LiL%20PEEP%20-%20nuts%20%28feat.%20lil%20skil%29%20%28Extended%29.mp3",file="cookies_nuts.mp3",soundName="COOKIES_NUTS",volume=0.75},
 {title="Cinderella",url="https://ia801000.us.archive.org/29/items/macmiller_202012/Cinderella.mp3",file="cookies_cinderella.mp3",soundName="COOKIES_Cinderella",volume=1,startAt=125},
 {title="WE ON GO",url="https://ia601009.us.archive.org/21/items/we-on-go/WE%20ON%20GO.mp3",file="cookies_we_on_go.mp3",soundName="COOKIES_WE_ON_GO",volume=0.8},
}

createTitle(SongPage,"♫ SONG","Toca una canción para reproducirla")
createInfo(SongPage,"Las canciones se descargan y se reproducen desde el archivo local.",Colors.RedLight)
...
local function validAudio(d)
 if type(d)~="string" or #d<10000 then return false end
 local h=d:sub(1,128):lower()
 if h:find("<html",1,true) or h:find("<!doctype",1,true) then return false end
 return d:sub(1,3)=="ID3" or d:byte(1)==255 or #d>100000
end
local function downloadAudio(url,file)
 if type(writefile)~="function" then return false end
 local req=request or http_request or (syn and syn.request) or (fluxus and fluxus.request)
 local data
 if type(req)=="function" then
  local ok,res=pcall(req,{Url=url,Method="GET",Headers={['User-Agent']='Mozilla/5.0',['Accept']='audio/mpeg,audio/*;q=0.9,*/*;q=0.8'}})
  if ok and type(res)=="table" then
   local code=tonumber(res.StatusCode or res.Status or res.status_code) or 0
   local body=res.Body or res.body
   if (code==0 or (code>=200 and code<300)) and validAudio(body) then data=body end
  end
 end
 if not data then
  local ok,res=pcall(function() return game:HttpGet(url,true) end)
  if ok and validAudio(res) then data=res end
 end
 if not data then return false end
 return pcall(writefile,file,data)
end
local function stopOthers(current)
 for _,s in pairs(SongStates) do
  if s~=current and s.sound then pcall(function() s.sound:Pause() end); s.playing=false; s.wanted=false end
 end
end
local function makeSound(cfg,state)
 if type(SongAsset)~="function" or not fileExists(cfg.file) then return false end
 local ok,id=pcall(SongAsset,cfg.file)
 if not ok or type(id)~="string" or id=="" then return false end
 if state.sound then pcall(function() state.sound:Destroy() end) end
 local snd=Instance.new("Sound")
 snd.Name=cfg.soundName
 snd.SoundId=id
 snd.Volume=cfg.volume or .75
 snd.Looped=true
 snd.Parent=SongParent
 state.sound=snd
 state.started=false
 return true
end
local function play(cfg,state)
 stopOthers(state)
 if not state.sound or not state.sound.Parent then if not makeSound(cfg,state) then return false end end
 if cfg.startAt and not state.started then pcall(function() state.sound.TimePosition=cfg.startAt end) end
 local ok=pcall(function() state.sound:Play() end)
 if ok then state.started=true;state.playing=true end
 return ok
end
local function prepare(cfg,state)
 if state.preparing then return end
 state.preparing=true
 if not makeSound(cfg,state) then
  if not fileExists(cfg.file) then downloadAudio(cfg.url,cfg.file) end
  if not makeSound(cfg,state) then state.preparing=false;return end
 end
 state.preparing=false
 if state.wanted then play(cfg,state) end
end
for _,cfg in ipairs(Songs) do
 local state={wanted=false,playing=false,started=false,preparing=false,sound=nil}
 SongStates[cfg.title]=state
 if fileExists(cfg.file) then task.defer(function() makeSound(cfg,state) end) end
 createToggle(SongPage,"♫ "..cfg.title,false,function(on)
  state.wanted=on
  if on then
   stopOthers(state)
   if state.sound and state.sound.Parent then play(cfg,state) else task.spawn(function() prepare(cfg,state) end) end
  else
   if state.sound then pcall(function() state.sound:Pause() end) end
   state.playing=false
  end
 end)
end
...
--// Infinite Jump (extracted)
local InfiniteJump = {
    running = false,
    connections = {},
}

local function applyInfiniteJumpBoost(boost)
    if not InfiniteJump.running then return end
...
local function enableInfiniteJump()
    if InfiniteJump.running then return end
    InfiniteJump.running = true

    table.insert(InfiniteJump.connections, UserInputService.JumpRequest:Connect(function()
        applyInfiniteJumpBoost(50)
    end))

    local holdJumpPressed = false
    local holdJumpActive = false

    table.insert(InfiniteJump.connections, UserInputService.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Keyboard
            ...
            task.delay(0.12, function()
                if InfiniteJump.running and holdJumpPressed then
                    holdJumpActive = true
                    applyInfiniteJumpBoost(50)
                end
            end)
    ...
    table.insert(InfiniteJump.connections, UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Keyboard
    ...
    table.insert(InfiniteJump.connections, RunService.Heartbeat:Connect(function()
        if InfiniteJump.running and holdJumpActive then
            applyInfiniteJumpBoost(50)
        end
    end))
end
...
local function disableInfiniteJump()
    InfiniteJump.running = false

    for _, conn in ipairs(InfiniteJump.connections) do
        pcall(function() conn:Disconnect() end)
    end
...
createTitle(
    DefensePage,
    "🛡️ DEFENSE",
    "Anti Bee • Anti Ragdoll • Infinite Jump • DROP BR"
)
...
local function buildPodiums()
    ...
    -- Crea una pieza sólida con colisión
    local function createVisual(root, cf, size, color, transparency)

        if not root or not PodiumHolder then
            return
        end

        local part = Instance.new("Part")

        part.Name = "SolidRainbowPodium"
        part.Size = size
    ...
    local function createOutline(part, color)
        ...
        if not part or not PodiumHolder then
            return
        end

        local outline = Instance.new("SelectionBox")

        outline.Name = "RainbowPodiumOutline"
    ...
    for _, plot in ipairs(Plots:GetChildren()) do
        ...
        if root and pods then
            ...
            for _, slot in ipairs(pods:GetChildren()) do
                ...
                if number then
                    ...
                    if cf and size then
                        ...
                        local color = getColor(number)

                        -- Convierte la posición global a posición relativa al root
                        local relativeCF =
                            root.CFrame:Inverse() * cf
            ...
            -- Podiums faltantes
            for i, data in ipairs(TEMPLATE) do

                if not pods:FindFirstChild(tostring(i)) then
...
top.Text = "⬇️  SIGUIENTE ⬇️"
top.Font = Enum.Font.GothamBlack
top.TextScaled = true
top.TextColor3 = Color3.fromRGB(0, 255, 255)    
top.TextStrokeColor3 = Color3.fromRGB(0, 255, 255) -- Color Aqua para el borde
...
ServerFPSLabel.TextSize = 12
ServerFPSLabel.Font = Enum.Font.GothamBlack

local serverFPSFrames = 0
local serverFPSLast = os.clock()

RunService.RenderStepped:Connect(function()
    serverFPSFrames += 1

    local now = os.clock()
    local elapsed = now - serverFPSLast

    if elapsed >= 0.5 then
        local fps = math.floor(serverFPSFrames / elapsed)
        serverFPSFrames = 0
        serverFPSLast = now

        ServerFPSLabel.Text = "FPS: " .. fps

        if fps >= 60 then
            ServerFPSLabel.TextColor3 = Colors.Green
        elseif fps >= 30 then
            ServerFPSLabel.TextColor3 = Colors.Yellow
        else
            ServerFPSLabel.TextColor3 = Colors.Red
        end
    end
end)
...
if RejoinButton then
    RejoinButton.BackgroundColor3 = Color3.fromRGB(220, 35, 45)
...
createInfo(
    SettingsPage,
    "PERFORMANCE • CANDY FPS BOOST + ANTI LAG",
    Colors.RedLight
)
...
--// ============================================================
--// FLOATING DROP BR BUTTON (MOBILE)
--// ============================================================

local DropBRButton = Instance.new("TextButton")
DropBRButton.Name = "DropBRFloatingButton"
DropBRButton.Size = UDim2.fromOffset(118, 50)
DropBRButton.Position = UDim2.new(1, -104, 1, -150)
DropBRButton.BackgroundColor3 = Colors.Background
DropBRButton.BorderSizePixel = 0
DropBRButton.Text = ""
DropBRButton.TextColor3 = Colors.White
DropBRButton.Font = Enum.Font.GothamBlack
DropBRButton.TextSize = 15
DropBRButton.TextStrokeTransparency = 0.15
DropBRButton.TextStrokeColor3 = Color3.fromRGB(255, 80, 220)
DropBRButton.AutoButtonColor = false
DropBRButton.Active = true
DropBRButton.ZIndex = 50
DropBRButton.Parent = ScreenGui

-- Neon title
local DropBRLabel = Instance.new("TextLabel")
DropBRLabel.Name = "NeonText"
DropBRLabel.Size = UDim2.new(1, -8, 1, -4)
DropBRLabel.Position = UDim2.fromOffset(4, 2)
DropBRLabel.BackgroundTransparency = 1
DropBRLabel.Text = "DROP BRAINROT"
DropBRLabel.Font = Enum.Font.GothamBlack
DropBRLabel.TextSize = 15
DropBRLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
DropBRLabel.TextStrokeTransparency = 0.05
DropBRLabel.TextStrokeColor3 = Color3.fromRGB(255, 60, 220)
DropBRLabel.TextXAlignment = Enum.TextXAlignment.Center
DropBRLabel.TextYAlignment = Enum.TextYAlignment.Center
DropBRLabel.ZIndex = 51
DropBRLabel.Active = false
DropBRLabel.Parent = DropBRButton

local DropBRTextGradient = Instance.new("UIGradient")
DropBRTextGradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 80, 230)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(120, 255, 255)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 80, 230)),
})
DropBRTextGradient.Parent = DropBRLabel

task.spawn(function()
    while DropBRLabel.Parent do
        DropBRTextGradient.Rotation = (DropBRTextGradient.Rotation + 2) % 360
        task.wait(0.03)
    end
end)

createCorner(DropBRButton, 14)
createStroke(DropBRButton, Colors.Red, 2)

local DropBRGradient = Instance.new("UIGradient")
DropBRGradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Colors.Red),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(80, 10, 20)),
    ColorSequenceKeypoint.new(1, Colors.Red),
})
DropBRGradient.Parent = DropBRButton

-- Touch-friendly floating button:
-- tap = execute, long-press + move = drag
local dropHolding = false
local dropMoved = false
local dropDragging = false
local dropStartInput
local dropStartPosition
local dropHoldStart = 0
local DROP_TAP_THRESHOLD = 12
local DROP_LONG_PRESS = 0.35

connect(DropBRButton.InputBegan, function(input)
    if input.UserInputType == Enum.UserInputType.Touch
        or input.UserInputType == Enum.UserInputType.MouseButton1 then
        dropHolding = true
        dropMoved = false
        dropDragging = false
        dropStartInput = input.Position
        dropStartPosition = DropBRButton.Position
        dropHoldStart = tick()
    end
end)

connect(UserInputService.InputChanged, function(input)
    if not dropHolding or not dropStartInput then return end
    if input.UserInputType ~= Enum.UserInputType.Touch
        and input.UserInputType ~= Enum.UserInputType.MouseMovement then return end

    local dx = input.Position.X - dropStartInput.X
    local dy = input.Position.Y - dropStartInput.Y
    local movedEnough = math.abs(dx) > DROP_TAP_THRESHOLD or math.abs(dy) > DROP_TAP_THRESHOLD
    local heldLongEnough = (tick() - dropHoldStart) >= DROP_LONG_PRESS

    -- Do not move the button until the finger has been held down.
    if heldLongEnough and movedEnough then
        dropDragging = true
        dropMoved = true
    end

    if dropDragging then
        DropBRButton.Position = UDim2.new(
            dropStartPosition.X.Scale,
            dropStartPosition.X.Offset + dx,
            dropStartPosition.Y.Scale,
            dropStartPosition.Y.Offset + dy
        )
    end
end)

connect(UserInputService.InputEnded, function(input)
    if not dropHolding then return end
    if input.UserInputType ~= Enum.UserInputType.Touch
        and input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end

    dropHolding = false
    if not dropDragging then
        playClick()
        task.spawn(runDrop)
    end
